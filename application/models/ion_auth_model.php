...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, created_on, first_name, last_name, password, active, last_login')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			$loggedin_group = (int) trim($this->input->post('role'));
			foreach ($user_groups as $key => $value) {				
				if($value->id==$loggedin_group){
					//User belongs to the logged in group
	...
	**/
	...
	{
		$group_id = (int) $this->input->post('role');
		if(!empty($group_id)){
		$group = $this->group($group_id)->result();
		...
		$session_data = array(
		    ...
		    'user_created'                => $user->created_on,
		    'old_last_login'       => $user->last_login,
		    'loggedin_group_name'       => $group['0']->name,
		    'loggedin_group_id'       => $group['0']->id
		);
	...
	public function remember_user($id)
	{
		...
		if ($this->db->affected_rows() > -1)
		{
			...
			set_cookie(array(
			    'name'   => $this->config->item('identity_group', 'ion_auth'),
			    'value'  => (int) trim($this->input->post('role')),
			    'expire' => $expire
			));
	...
	public function login_remembered_user()
	{
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', id, created_on, first_name, last_name, email, last_login')
